...
SELECT location,date,total_cases,total_deaths,(total_deaths/total_cases)*100 AS DeathPercentage
  ...
  -- 2) looking for high amount of death with covid
 ...
 group by continent
  order by  TOTALDEATHCOUNT DESC;
   ...
   -- 3) looking of country with high infection rate
 ...
 -- WHERE continent is not null
  ...
  -- 4) Percent Population Infected
   SELECT location,date,population,max(total_cases) as Highinfectioncount,max((total_cases/population)*100) AS PercentPopulationInfected
  FROM [dbo].[CovidDeath]
 -- WHERE continent is not null
